  ...
  <div>
    ...
    <b-card no-body class="mb-0">
      ...
      >
        ...
        <template #cell(user)="data">
          <b-media vertical-align="center">
            ...
            <small class="text-muted">{{
              data.item.institute != null && data.item.profession
                ? `${data.item.profession} di ${data.item.institute}`
                : '-'
            }}</small>
          </b-media>
        </template>
        ...
        <!-- Column Verified -->
        <template #cell(emailVerifiedAt)="data">
          <b-badge
            pill
            :variant="
              `light-${data.item.emailVerifiedAt ? 'success' : 'secondary'}`
            "
            class="text-capitalize"
          >
            {{ data.item.emailVerifiedAt ? 'Verified' : 'Unverified' }}
          </b-badge>
        </template>
  ...
  data() {
    return {
      ...
      tableColumns: [
        ...
        { key: 'emailVerifiedAt', sortable: true, label: 'Status' },
        { key: 'actions' },
      ],
      orderBy: 'created_at',
      orderType: 'desc',
  ...
  methods: {
    async loadUsers() {
      try {
        ...
        this.totalUsers = response.data.meta.pagination.total
        ...
        this.$swal({
          ...
          if (result.value) {
            const response = await useJwt.axiosIns.delete(
              `${process.env.VUE_APP_API_BASE_URL}/users/${id}`
            )
