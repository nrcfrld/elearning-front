  ...
  data() {
    return {
      ...
      tableColumns: [
        ...
        { key: 'order.status', label: 'Order Status', sortable: true },
        { key: 'order.amount', label: 'Order Amount', sortable: true },
    ...
    async loadData() {
      try {
        ...
        const response = await useJwt.axiosIns(
          `${process.env.VUE_APP_API_BASE_URL}/payment-logs`,
          {
            params: {
        ...
        this.$swal({
          ...
          if (result.value) {
            const response = await useJwt.axiosIns.delete(
              `${process.env.VUE_APP_API_BASE_URL}/categories/${id}`
            )
